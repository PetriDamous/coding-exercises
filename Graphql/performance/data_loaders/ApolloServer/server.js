...
async function apolloServer() {
  const context = () => {
    return {
      roomLoader: new DataLoader(async (roomIds) => {
        ...
        const roomMap = {}; // creates an object map so it looks like {"1": {room returned from data object}}
        data.forEach((room) => {
          return (roomMap[room.id] = room);
        });

        // The map then is used in an object lookup pattern to
        // accuratly produce an array that is mapped from the data array and
        // the roomIds array that is passed in from the paramter.
  ...
  const server = new ApolloServer({
    context, // into our apollo config
    typeDefs,
    resolvers,
  });
